...
try {
    ...
    if (!in_array($data['gender'], ['L', 'P'])) {
        echo json_encode(['error' => 'Nilai gender tidak valid. Hanya diperbolehkan "L" atau "P".']);
        exit();
    }
    
    if (!isset($data['nidn'], $data['email'], $data['gender'], $data['phone'])) {
        echo json_encode([
    ...
    $query = "
        UPDATE Pegawai
        SET nidn = ?, email = ?, jk_pegawai = ?, nohp_pgw = ?
        WHERE id_pegawai = ?
    ";
    $params = [
        $data['nidn'],
